...
if __name__ == '__main__':
  ...
  class GenerateQuestion(object):
    def __init__(self,seed=None,quiet=False,answer='saved'):
      if seed:
        random.seed = seed
  ...
  class GradingSystem(object):
    def __init__(self,quiet=False,seed=1):
      self.quiet = quiet
      self.seed = seed
      # Collected info 
      self.contain = ''
    ...
    def PossibleWords(self):
      ...
      for word in self.length_5:
        ...
        for j,char in enumerate(word):
          ...
          green = self.weightedFrequency[self.alphabet.index(char),j]/self.size_5 
          # chance of getting a yellow letter
          yellow = (np.sum(self.weightedFrequency[self.alphabet.index(char)]) - green*self.size_5)/(self.size_5*4.)
  ...
  if len(PickAAnswer) == 0:
    Question = GenerateQuestion(seed=777)
  else:
    Question = GenerateQuestion(seed=None,answer=PickAAnswer.lower())
  print('The program picked [%s]' % Question.answer)

  # now iteration
  words = 10
  ob = GradingSystem(quiet=True)
  words = ob.PossibleWords()
